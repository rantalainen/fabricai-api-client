...
import { IFabricAiApiClientConfig, IFabricAiApiClientOptions } from './interfaces';
...
export class FabricAiApiClient {
  options: IFabricAiApiClientOptions;
  config: Omit<IFabricAiApiClientConfig, 'keepAliveAgent' | 'dnsCache'>;
  readonly api: FabricAiApiClientInstance;

  constructor(options: IFabricAiApiClientOptions, config: IFabricAiApiClientConfig = {}) {
    // Set default config
    config.baseURL = config.baseURL || 'https://ai.fabricai.io';
    config.timeout = config.timeout || 120000;

    if (!options.apiKey) {
      throw new Error('FabricAI error: Missing options.apiKey');
    }
    ...
    this.api = new FabricAiApiClientInstance({
      ...this.config,
  ...
  private installErrorHandler() {
    this.api.instance.interceptors.response.use(
      ...
      (error) => {
        error.message =
          `FabricAI HTTP error ${error.response.status} (${error.response.statusText}): ` + JSON.stringify(error.response.data);
        throw error;
      }
    );
  }
...
class FabricAiApiClientInstance extends Api<any> {
